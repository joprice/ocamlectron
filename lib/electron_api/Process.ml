...
let allowed_node_environment_flags () =
  ...
  let () = Set.iteri (fun i value -> arr.(i) <- Js.to_string value) set in
  arr

let extract_bool result = Optdef.get result (fun () -> false)
...
let argv () =
  ...
  raw_argv |> Js.to_array |> Array.map Js.to_string
...
let kill ?signal pid =
  let sign =
    Option.(signal >|= Signal.to_string >|= Js.string) |> Optdef.from_option
  in
