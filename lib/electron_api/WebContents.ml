...
type _ event =
  ...
  | DidFailLoad
      : (Event.js ->
        ...
        unit)
        event
  | DidFrameFinishLoad : (Event.js -> bool Js.t -> int -> int -> unit) event
  | DidStartLoading : (Event.js -> unit) event
  | DidStopLoading : (Event.js -> unit) event
  | DomReady : (Event.js -> unit) event
  | PageFaviconUpdated
      : (Event.js -> Js.js_string Js.t Js.js_array Js.t -> unit) event
  | NewWindow
      : (Event.js ->
        Js.js_string Js.t ->
        Js.js_string Js.t ->
        ...
        unit)
        event
  | WillNavigate : (Event.js -> Js.js_string Js.t -> unit) event
  | DidStartNavigation : (Event.js -> Js.js_string Js.t -> unit) event
  | DidNavigate
      : (Event.js -> Js.js_string Js.t -> int -> Js.js_string Js.t -> unit)
        event
  | DidFrameNavigate
      : (Event.js ->
        Js.js_string Js.t ->
        ...
        event
  | DidNavigateInPage
      : (Event.js -> Js.js_string Js.t -> bool Js.t -> int -> int -> unit) event
  ...
  | PluginCrashed
      : (Event.js -> Js.js_string Js.t -> Js.js_string Js.t -> unit) event
  ...
  | DidChangeThemeColor : (Event.js -> Js.js_string Js.t Js.Opt.t -> unit) event
  | UpdateTargetUrl : (Event.js -> Js.js_string Js.t Js.Opt.t -> unit) event
  | ContextMenu
      : (Event.js ->
...
let reload ?(ignoring_cache = false) win =
  if ignoring_cache then win##reloadIgnoringCache () else win##reload ()
...
let replace ?(misspelling = false) win text =
  ...
  if misspelling then win##replaceMisspelling str else win##replace str

let paste ?(match_style = false) win =
  if match_style then win##pasteAndMatchStyle () else win##paste ()

let to_opt f x = Optional.Option.map x f |> Js.Optdef.option
...
let find_in
  ...
  in
  win##findInPage (Js.string words) (Js.Optdef.return opts)

let printers win =
  win##getPrinters ()
  |> Tools.js_array_to_list Electron_plumbing.Struct.PrinterInfo.from_object

let print ?silent ?background ?device ?callback win =
  let cb =
    to_opt (fun f -> Js.wrap_callback (fun x -> f (Js.to_bool x))) callback
  in
  let op =
    object%js
...
let save_page ~path ~save_type ~callback win =
  ...
  win##savePage p (Js.string st) cb |> Js.to_bool

let show_definition_for_selection win = win##showDefinitionForSelection ()

let is_off_screen win = Js.to_bool (win##isOffscreen ())

let start_painting win = win##startPainting ()

let stop_painting win = win##stopPainting ()

let is_painting win = Js.to_bool (win##isPainting ())

let frame_rate win fps = win##setFrameRate fps

let frame_rate_of win = win##getFrameRate ()

let invalidate win = win##invalidate ()

let id_of win = win##.id

let os_process_id_of win = win##getOSProcessId ()
