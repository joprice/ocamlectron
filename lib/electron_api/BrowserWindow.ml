...
let make
    ...
    ?tabbing_identifier
    electron =
  Electron_plumbing.Struct.BrowserWindow.make ?width ?height ?position
    ?use_content_size ?center ?min_width ?min_height ?max_width ?max_height
    ?resizable ?movable ?minimizable ?maximizable ?closable ?focusable
    ?always_on_top ?fullscreen ?fullscreenable ?simple_fullscreen ?skip_taskbar
    ?kiosk ?title ?show ?frame ?parent ?modal ?accept_first_mouse
    ?disable_auto_hide_cursor ?auto_hide_menu_bar ?enable_larger_than_screen
    ?background_color ?has_shadow ?opacity ?dark_theme ?transparent ?_type
    ~title_bar_style:(title_bar_style_to_string title_bar_style)
    ?fullscreen_window_title ?thick_frame
    ~vibrancy:(vibrancy_to_string vibrancy)
    ?zoom_to_page_width ?tabbing_identifier
    electron##._BrowserWindow_fromOpts

let all electron =
  (singleton electron)##getAllWindows () |> Js.to_array |> Array.to_list

let focused electron =
  (singleton electron)##getFocusedWindow () |> Js.Opt.to_option
...
let show ?(focus = true) win =
  if focus then win##show () else win##showInactive ()
...
let aspect_ratio ?extra_size win ratio =
  ...
  let size =
    extra_size >|= Electron_plumbing.Struct.Size.to_object |> to_optdef
  in
  win##setAspectRatio ratio size
...
let bounds_of win =
  win##getBounds () |> Electron_plumbing.Struct.Rectangle.from_object

let content_bounds_of win =
  win##getContentBounds () |> Electron_plumbing.Struct.Rectangle.from_object
...
let always_on_top ?(level = `Floating) ?(relative_level = 0) win flag =
  ...
  win##setAlwaysOnTop (Js.bool flag) (Js.Optdef.return l)
...
let is_kiosk win = win##isKiosk () |> Js.to_bool
...
let parent_of win = win##getParentWindow () |> Js.Opt.to_option

let childs_of win = win##getChildWindows () |> Js.to_array |> Array.to_list

let web_contents win = win##.webContents
