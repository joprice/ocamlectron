...
module Make (F : REQ) : OPTION with type 'a t = 'a F.t = struct
  include F

  let to_bool x = case x (fun () -> false) (fun _ -> true)
...
module Option : OPTION with type 'a t = 'a option = Make (struct
  type 'a t = 'a option
...
module Opt : OPTION with type 'a t = 'a Js.Opt.t = Make (struct
  include Js.Opt
...
module Optdef : OPTION with type 'a t = 'a Js.Optdef.t = Make (struct
  include Js.Optdef
