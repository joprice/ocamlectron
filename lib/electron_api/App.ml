...
let quit app = app##quit ()

let exit ?code app =
  let exit_code = Option.to_optdef code in
  app##exit exit_code

let relaunch app = app##relaunch Optdef.empty

let relaunch_with ~args ?exec_path app =
  let a = Js.string args in
  let path = Js.string |> Option.map exec_path |> Option.to_optdef in
  let options =
    object%js
      val args = a
  ...
  in
  app##relaunch (Optdef.pure options)
...
let clear_recent_documents app = app##clearRecentDocuments ()
...
let set_as_default_protocol_client_with ?path ?args app protocol_name =
  ...
  let a =
    Option.(
      args >|= List.map Js.string >|= Array.of_list >|= Js.array |> to_optdef)
  in
  let result = app##setAsDefaultProtocolClient protocol p a in
  Js.to_bool result
...
let remove_as_default_protocol_client_with ?path ?args app protocol_name =
  ...
  let a =
    Option.(
      args >|= List.map Js.string >|= Array.of_list >|= Js.array |> to_optdef)
  in
  let result = app##removeAsDefaultProtocolClient protocol p a in
  Js.to_bool result
...
let is_default_protocol_client_with ?path ?args app protocol_name =
  ...
  let a =
    Option.(
      args >|= List.map Js.string >|= Array.of_list >|= Js.array |> to_optdef)
  in
  let result = app##isDefaultProtocolClient protocol p a in
  Js.to_bool result
...
let make_single_instance app f =
  let pre_f args workdir =
    ...
    f arguments wd
  in
  let result = app##makeSingleInstance (Js.wrap_callback pre_f) in
  Js.to_bool result
