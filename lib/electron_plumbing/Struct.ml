...
module Versions = struct
  type t = { chrome : string; electron : string }

  let from_object obj =
    { chrome = Js.to_string obj##.chrome; electron = Js.to_string obj##.chrome }
end

module CPUUsage = struct
  type t = { percent_cpu_usage : int; idle_wakeups_per_second : int }
...
module ProcessMemoryInfo = struct
  type t = {
...
module SystemMemoryInfo = struct
  type t = { total : int; free : int; swap_total : int; swap_free : int }
...
module MemoryUsage = struct
  type t = { rss : int; heap_total : int; heap_used : int; external_ : int }
...
module Release = struct
  type lts = [ `Argon | `Boron | `Carbon | `Unknown of string ]
  ...
  let from_object obj =
    {
      ...
      lib_url =
        Js.Optdef.(obj##.libUrl |> (fun x -> map x Js.to_string) |> to_option);
      lts =
        Js.Optdef.(
          obj##.lts
...
module Size = struct
  ...
  let from_object obj = { width = obj##.width; height = obj##.height }
...
module Position = struct
  ...
  let from_object obj = { x = obj##.x; y = obj##.y }
...
module Error = struct
  type t = { message : string; code : string; stack : string }
